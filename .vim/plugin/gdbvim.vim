...
let g:loaded_gdbvim_mappings = 0
...
function Gdb_interf_close()
    ...
    let s:BpSet = ""
    let s:having_partner=0
	if g:loaded_gdbvim_mappings == 1
		call s:Gdb_unshortcuts()
		let g:loaded_gdbvim_mappings = 0
	endif
endfunction
...
" Toggle breakpoints
function Gdb_ttogglebreak(name, line)
     silent call Gdb_command("tbreak ".a:name.":".a:line)
     silent call Gdb_command("continue ")
endfun
...
function s:Gdb_shortcuts()
    nmap <silent> <F9>          :call Gdb_togglebreak(bufname("%"), line("."))<CR>
    nmap <silent> <F10>         :call Gdb_ttogglebreak(bufname("%"), line("."))<CR>
    nmap <silent> <C-F5>        :Gdb run<CR>
    nmap <silent> <F7>          :Gdb step<CR>
    nmap <silent> <F8>          :Gdb next<CR>
    nmap <silent> <F6>          :Gdb finish<CR>
    nmap <silent> <F5>          :Gdb continue<CR>
    nmap <silent> <C-p>         :call Gdb_command("print ".expand("<cword>"))<CR> 
    nmap <silent> <C-i>         :call Gdb_command("print *".expand("<cword>"))<CR> 
    nmap <silent> <C-w>         :call Gdb_command("watch ".expand("<cword>"))<CR> 
    nmap <silent> <C-u>         :Gdb up<CR>
    nmap <silent> <C-d>         :Gdb down<CR>
    call s:InitMenu()
endfunction

function s:Gdb_unshortcuts()
    nunmap  <F9>
    nunmap  <F10>
    nunmap  <C-F5>
    nunmap  <F7>
